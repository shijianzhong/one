    ...
    pub fn with_workspace(
        ...
        let system_prompt = format!(
            "{}\n\n当前日期：{}\n操作环境：{}\n\n请严格按照上述灵魂设定和准则行动。\n\n\
             ...
             需要执行普通非编码 shell 命令时，使用 run_in_terminal。普通命令会在右侧终端的 Shell tab 独立执行，不要发送到 Claude/Codex/Gemini 的 Coding tab，也不要打断正在运行的编码 runtime。命令执行后应根据结果向用户做简洁说明，而不是要求用户自己去终端理解输出。",
            soul_content,
