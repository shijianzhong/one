...
- [ ] 远程触发自动锁 PermissionMode = Strict；危险操作双确认。
...
- **2026-06-04 M3-1**：M3 远程触达骨架——新增 `src/triggers/` 模块（`Trigger` trait + `TriggerEvent / TriggerReply`），`triggers/dispatcher.rs` 纯 tokio 解析 `/help` `/skills` `/preview <id> [json]` `/run <id> [json]` `/audit [n]` 命令并直调 `crate::skills::registry()` + `crate::task_db::load_recent_run_events`（不持任何 GPUI 句柄）；`triggers/telegram.rs` 用既有 `reqwest 0.12` 跑 50s long-poll `getUpdates`，按 `ONE_TELEGRAM_BOT_TOKEN` + `ONE_TELEGRAM_ALLOWED_CHATS` 启动+白名单，`/run` 通过 `Skill::execute` 自然走到本机 PermissionPolicy 弹窗形成"远程一次 + 本机一次"双确认，未授权 chat_id 直接拒绝并日志告警；`task_db.rs` 新增 `RunEventRow` + `pub fn load_recent_run_events(limit)`（开新 `Connection::open_file(get_db_path())` 走 sqlez `Statement::map`），`main.rs` `gpui_tokio::init` 之后 `TelegramTrigger::from_env()` 命中时 `std::thread::spawn` + `tokio::runtime::Runtime::new()` 后台 `block_on(trigger.run())`，没设 token 时静默跳过保留主进程纯 GPUI 行为不变。`cargo check` 干净，`cargo test` 32/32 通过（含 7 个新增 triggers 测试）。M3 第一刀闭环，剩"远程来源自动锁 Strict"留给 M3-2。
